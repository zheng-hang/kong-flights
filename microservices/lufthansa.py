    ...
    try:
        if 'Flight' in schedule:  # Check if schedule is a single flight
            ...
            try:
                ...
                print(f"IATA code not found in pyairports database: {dep_airport_code}")
                departure_loc = "Unknown"
            try:
                arrival_loc = airports.airport_iata(arr_airport_code)[1]
            except AirportNotFoundException:
                print(f"IATA code not found in pyairports database: {arr_airport_code}")
                arrival_loc = "Unknown"
